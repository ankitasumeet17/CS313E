...
#  Description: this program midifies the sub_sets() function that we wrote in class, 
#               adding additonal constraints and sorting intergers into sets, and retruning a boolean.

#  Student Name: Ivy (Jingwen) Lou

#  Student UT EID: Jl75477

#  Partner's Name:  Ankita Sumeet

#  Partner's UT EID: as96977
...
def groupSum6(start, nums, target):
    if start >= len(nums):
        ...
        if nums[start] == 6:        #if equals 6, add to group
...
def groupSumClump(start, nums, target):
    ...
    while(i < len(nums) and nums[start] == nums[i]):
        sum += nums[i]
        i += 1                         
    return (groupSumClump(i, nums, target - sum)) or (groupSumClump(i, nums, target))
...
# (No loops needed.)
def splitArray(nums):
    return splitArrayHelper(0,nums,0,0) #call helper function, set start, set1, set2 to 0


def splitArrayHelper(start, nums, sum_1set, sum_2set):
    # start = 0
    # sum_1set = 0
    # sum_2set = 0

    if start >= len(nums):
        return sum_1set == sum_2set #base case
    return splitArrayHelper(start + 1, nums, sum_1set + nums[start], sum_2set) or splitArrayHelper(start + 1, nums,sum_1set , sum_2set+ nums[start])
...
def splitOdd10(nums):
    return splitOdd10Helper(0,nums,0,0) #call helper function, set start, set1, set2 to 0

  
def splitOdd10Helper(start, nums, sum_1set, sum_2set):
    if start >= len(nums):
        return sum_1set % 10 == 0 and sum_2set % 2 == 1 #base case
    return splitOdd10Helper(start+1, nums, sum_1set + nums[start], sum_2set) or splitOdd10Helper(start+1, nums, sum_1set, sum_2set + nums[start])
...
def split53(nums):
    return split53Helper(0,nums,0,0) #call helper function, set start, set1, set2 to 0

def split53Helper(start, nums, sum_1set, sum_2set):
    if start >= len(nums):
        return sum_1set == sum_2set #base case 

    if nums[start] % 5 == 0:   # if is multiple of 5
        return split53Helper(start + 1, nums, sum_1set + nums[start], sum_2set)
    elif nums[start] % 3 == 0:   # if is multiple of 3
        return split53Helper(start + 1, nums, sum_1set, sum_2set+ nums[start])
    else:
        return split53Helper(start+1, nums, sum_1set + nums[start], sum_2set) or split53Helper(start+1, nums, sum_1set, sum_2set + nums[start])
